...
def kfold_cv(model, xFeat, y, k):
    ...
    trainAuc, testAuc = 0, 0
    # Splitting data into k splits
    for train_index, test_index in kf.split(xFeat):
        xTrain, xTest = xFeat[train_index], xFeat[test_index]
        yTrain, yTest = y[train_index], y[test_index]
        # Train and get AUC for each split
        train_auc, test_auc = sktree_train_test(model, xTrain, yTrain, xTest, yTest)
        trainAuc += train_auc
        testAuc += test_auc
    # Get average AUC
    trainAuc /= k
...
def sktree_train_test(model, xTrain, yTrain, xTest, yTest):
    ...
    fpr, tpr, threshold = metrics.roc_curve(yTrain, yHatTrain[:, 1])
